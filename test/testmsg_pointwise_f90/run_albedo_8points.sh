...
echo "testing msg albedo on eight points, using fortran namelist as input"
...
for i in "${!xList[@]}"; do
  ...
  python3 ../../pyal2/wrapper.py acf.msg.model0.startseries.true pcf.msg.2016-08-01 -i MSG -f f90nml_msg --outputdates 2016-08-01 --keywords name point${i} --loglevel debug -x ${xList[i]} ${xList[i]} -y ${yList[i]} ${yList[i]} --debuglevel 10000 1>log/log.1.${i}  2>log/logerr.1.${i}
  python3 ../../pyal2/wrapper.py acf.msg.model0.startseries.false pcf.msg.2016-08-02 -i MSG -f f90nml_msg --outputdates 2016-08-02 --keywords name point${i} --loglevel debug -x ${xList[i]} ${xList[i]} -y ${yList[i]} ${yList[i]} --debuglevel 10000 1>log/log.2.${i}  2>log/logerr.2.${i}
  ...
  echo "python3 ../../pyal2/validation/check_tests_msg.py --old ./reference-data --new output-point${i} -x ${xList[i]} ${xList[i]} -y ${yList[i]} ${yList[i]}  --date 20160801" 1>>log/log.diff  2>>log/logerr.diff
  ...
  echo "python3 ../../pyal2/validation/check_tests_msg.py --old ./reference-data --new output-point${i} -x ${xList[i]} ${xList[i]} -y ${yList[i]} ${yList[i]} --date 20160802" 1>>log/log.diff  2>>log/logerr.diff
